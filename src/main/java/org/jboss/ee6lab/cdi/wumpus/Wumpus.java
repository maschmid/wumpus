...
public class Wumpus {
	...
	public void onShoot(@Observes PlayerShootAtRoomEvent event, GameMessage gameMessage, @Current Room currentRoom, @Current Player currentPlayer, @Current List<Player> players) {
		if (event.getRoom() == room) {
			...
			for (Player p : players) {
				if (p != currentPlayer) {
					p.setAlive(false);
					p.setDeathMessage("Player " + currentPlayer.getName() + " killed the wumpus and won the game!");
				}
			}
		}
	}
	...
	private boolean isWumpusNearby(Room otherRoom) {
		...
		if (otherRoom == room || otherRoom.getNorth() == room || otherRoom.getSouth() == room || otherRoom.getEast() == room || otherRoom.getWest() == room) {
			return true;
		}
	...
	public void onMove(@Observes @ObserverOrder(5) PlayerEnteredRoomEvent event, GameMessage gameMessage, @Current Room currentRoom, @Current Player currentPlayer) {
		...
		// Wumpus smells really bad, you can smell it from the distance of two rooms! 
